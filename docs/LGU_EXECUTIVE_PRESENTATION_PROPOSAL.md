...
```mermaid
flowchart TD
    A([💰 Budget Allocation]) -->|Assigned to specific Project/Office| B(📈 Project Budget Ceiling Set)
    B -->|Authorized Purchase Made| C(🧾 Receipt Scanned via Phone/PC)
    C -->|Uploaded with details| D(🛡️ Admin/Treasurer Verification)
    D -->|Approved & Saved| E(📊 Live Balance Automatically Updated)
    E --> F[📱 Public Transparency Dashboard]
    E --> G[📑 1-Click COA / Audit Export Packet]
    E --> H[🔒 Permanent Immutable History Log]
